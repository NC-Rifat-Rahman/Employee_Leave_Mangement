...
public class LeaveApplicationServiceImpl implements LeaveApplicationService
{
    ...
    private LeaveApplication getLeaveApplicationFromMap(Map<String,String> requestMap,Boolean isAdd)
    {
        ...
        //leaveApplication.setFromDate(LocalDateTime.parse("from_date"));
        //leaveApplication.setFromDate(LocalDateTime.parse("to_date"));
    ...
    public ResponseEntity<List<LeaveApplication>> getLeaveApplicationByLeaveType(String name)
    {
        ...
        try
        {
            if(jwtFilter.isUser())
            {
                return new ResponseEntity<>(leaveApplicationRepository.findByLeaveTypeName(name),HttpStatus.OK);
            }
        }
        catch (Exception ex)
        {
    ...
    public List<LeaveApplication> getLeaveApplicationByLeaveTypeStatus(String status)
    {
        ...
        {
            if(jwtFilter.isUser())
                return leaveApplicationRepository.findByLeaveTypeStatus(status);
        }
        catch (Exception ex)
        {
            ex.printStackTrace();
    ...
    {
        if(jwtFilter.isUser())
            return leaveApplicationRepository.findByLeaveApplicationDate(date1,date2);
        return null;
    }
